...
def tonight(request):
	events = Events.objects.today().filter(latest=True)
	context = {
		'events': events,
	}
...
def archive(request):
	events = Events.objects.filter(latest=True)
	context = {
		'events': events,
	}
	return render_to_response(
...
def toggle_attendance(request):
    ...
    attendance, created = Attendance.objects.get_or_create(user=request.user,
                                                           event=event)
    if created:
        request.user.message_set.create(message='You are now attending "%s"' %
                                        event)
    else:
        attendance.delete()
        request.user.message_set.create(message='You are no longer attending "%s"' % 
                                        event)
